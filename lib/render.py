...
from torchvision.transforms import GaussianBlur as gaussian
...
class Rasterizer:
    ...
    def __init__(
        ...
        self._filter = gaussian([3, 3], sigma=0.5)
    ...
    def render_texture_albedo(self, 
        ...
        # albedo = dr.texture(texture["albedo"][0], texc, texd, mip=texture["albedo"][1:], filter_mode='linear-mipmap-linear')
        if isinstance(texture["albedo"], list):
            albedo = dr.texture(texture["albedo"][0], texc, filter_mode='linear')
            # albedo = dr.texture(texture["albedo"][0][None, ...], texc, texd, filter_mode='linear-mipmap-linear', max_mip_level=6)
        else:
